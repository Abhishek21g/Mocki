...
function SetupPage() {
  ...
  const { getAccessToken, status, user, signInWithGitHub, signInWithMagicLink } = useSupabaseAuth();
  ...
  if (status === "ready" && !user) {
    return <LoginPage signInWithGitHub={signInWithGitHub} signInWithMagicLink={signInWithMagicLink} />;
  }
...
function LoginPage({ signInWithGitHub, signInWithMagicLink }: {
          ...
          <div className="flex flex-col gap-4">
            ...
            {inApp ? (
              <div
                ...
                style={{ opacity: 0.4, cursor: "not-allowed", justifyContent: "center" }}
              >
                Continue with Google (not supported in app)
              </div>
            ) : (
              <GoogleSignInButton
                text="continue_with"
            ...
            <button
              ...
              disabled={inApp}
              onClick={() => !inApp && signInWithGitHub()}
            >
