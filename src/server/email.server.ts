...
export async function sendCheckInEmail(email: string, name: string): Promise<{ ok: boolean; error?: string }> {
  ...
  // Plain-text style HTML — no marketing layout, no gradients, no tables.
  // Gmail scores these as personal emails and routes to Primary.
  const html = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
<body style="margin:0;padding:0;background:#ffffff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#111111;">
  <div style="max-width:520px;margin:40px auto;padding:0 24px;">
    ...
    <p style="margin:0 0 16px;font-size:15px;line-height:1.7;color:#111;">
      You signed up for Mocki a little while ago but haven't run an interview yet — wanted to check in.
    </p>

    <p style="margin:0 0 16px;font-size:15px;line-height:1.7;color:#111;">
      It takes about 15 minutes. Paste in a job description and your resume, and three AI interviewers will go through the full round with you — follow-up questions, pushbacks, the whole thing. At the end you get a score and a breakdown of exactly where you need to improve.
    </p>

    <p style="margin:0 0 28px;font-size:15px;line-height:1.7;color:#111;">
      <a href="https://mocki.dev" style="color:#111;font-weight:600;">Give it a shot →</a>
    </p>
...
Give it a shot: https://mocki.dev
